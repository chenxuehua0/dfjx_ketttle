...
function jobCompositionImg(){
    var grid=Ext.getCmp("JobPanel");
    var jobId=grid.getSelectionModel().getSelected().get("jobId");
    var jobName=grid.getSelectionModel().getSelected().get("name");//被选中的任务路径
    
    var imgSrc=GetUrl('task/getJobImage.do?jobId='+jobId);
    var htmlInfo='<div style="height:470px;width:770px;overflow:auto;"><img src="'+imgSrc+'"></div>';
    var jobImgWindow=new Ext.Window({
        id:"jobImgWindow",
        title:jobName,
